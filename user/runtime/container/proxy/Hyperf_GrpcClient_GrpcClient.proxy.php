...
class GrpcClient
{
    ...
    public function set(string $hostname, array $options = [])
    {
        ...
        $defaultOptions = ['timeout' => self::GRPC_DEFAULT_TIMEOUT, 'send_yield' => false, 'ssl' => false, 'ssl_host_name' => '', 'credentials' => null];
     ...
     * @param mixed $data
     */
    public function openStream(string $path, $data = '', string $method = '', bool $usePipelineRead = false, array $metadata = []) : int
    {
    ...
    public function send(Request $request) : int
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['request'], 'keys' => compact(['request']), 'variadic' => ''], function (Request $request) use($__function__, $__method__) {
            if (!$this->isConnected()) {
                return 0;
            }
    ...
    {
        if ($this->sendYield === true) {
            return $this->sendChannel->push([$streamId, $data, $end]) && $this->sendResultChannel->pop();
        }
    ...
    public function recv(int $streamId, ?float $timeout = null)
    {
        $__function__ = __FUNCTION__;
        $__method__ = __METHOD__;
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['streamId', 'timeout'], 'keys' => compact(['streamId', 'timeout']), 'variadic' => ''], function (int $streamId, ?float $timeout = null) use($__function__, $__method__) {
